...
def pre_process_data_3():
    ...
    df = sales.groupby([sales.date.apply(lambda x: x.strftime('%Y-%m')), 'item_id', 'shop_id']).sum().clip(0,20).reset_index()
...
test['item_cnt_day'] = test['item_cnt_day'].clip(0, 20)
test[['ID', 'item_cnt_day']].to_csv('ver_%d.csv' % ver, index=False)
